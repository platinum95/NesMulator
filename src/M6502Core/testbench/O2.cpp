...
void CPU::fetch()
{
	...
	if (addm == IMM || addm == IMPLIED || opcode == 0x20 ) {
		state = EXECUTE;
	} else {
		state = LOAD;
	}
...
void CPU::exec()
{
	...
	switch (opcode) {
		...
		/* Stack instructions */
		case 0x9A: return o_tr(S, X, false);
...
{
	switch (cycle_count) {
		case 0: wv0 = rd(av++); ++PC; break;
		case 1: wv1 = rd(av); ++PC; break;
...
void CPU::o_brk()
{
	switch ( cycle_count ) {
		case 0: ++PC; break;
		case 1: push( PC >> 8 ); break;
		case 2: push( PC & 0x00FF );break;
		case 3: push( P_CHAR | ( 1 << 4 ) | ( 1 << 5 ) ); P[INTERRUPT] = true; break;
		case 4: PC = ( PC & 0xFF00 ) | rd( 0xFFFE ); break;
		case 5: PC = ( PC & 0x00FF ) | ( rd( 0xFFFF ) << 8 ); state = FETCH; break;
	}
}
void CPU::o_dec()
{
     ...
     JSR
       ...
       --- ------- --- -------------------------------------------------
        ...
        6    PC     R  copy low address byte to PCL, fetch high address
                       byte to PCH
	*/
	std::cout << "JSR exec cycle: " << static_cast<int>( cycle_count ) << std::endl;
	switch ( cycle_count ) {
		case 0:
			wv0 = rd( PC++ );
		break;
...
void CPU::o_rti()
{
	...
	switch (cycle_count) {
		...
		case 2:
			...
			++S;
			break;
		case 3: PC = ( PC & 0xFF00 ) | rd( 0x0100 + S++ ); break;
		case 4:
			PC	  = ( PC & 0x00FF ) | ( rd( 0x0100 + S ) << 8 );
			state = FETCH;
			break;
		default: break;
	}
...
void CPU::o_rts()
{
	switch (cycle_count) {
		case 0: break;
		case 1: ++S; break;
		case 2: PC = ( PC & 0xFF00 ) | rd( 0x100 + ( S++ ) ); break;
		case 3: PC = ( rd( 0x0100 + S ) << 8 ) | ( PC & 0x00FF ); break;
		case 4:
			++PC;
			state = FETCH;
			break;
...
void CPU::o_tr(unsigned char& reg0, unsigned char reg1, bool updateStatus)
{
	reg0 = reg1;
	if ( updateStatus )
		flUpdate(reg0);
